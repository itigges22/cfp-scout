    ...
    <Section title="At a glance">
      <div className="grid grid-cols-2 gap-x-6 gap-y-2 text-sm">
        ...
        <Stat
          ...
          value={
            g.series
              ? `Edition of ${g.series.canonical_name} · DAAM attended ${g.series.daam_attended_recent} of past ${g.series.past_editions_count}`
              : "—"
          }
        />
      </div>
...
function PastEngagement({ brief }: { brief: ConferenceBrief }) {
  if (brief.past_engagement.length === 0) {
    return (
      <Section title="Past DAAM engagement">
        <Empty>No prior DAAM attendance recorded for this series.</Empty>
      </Section>
    );
  }
  return (
