...
function SmesPage() {
  ...
  const teamParam = teamFilter === "daam" ? "DAAM" : undefined;
  const query = useQuery({
    ...
    // Non-DAAM filter is harder server-side (NOT match) — we'll fetch all and
    // filter client-side for now. Cheap because the SME table stays small.
    select: (data) =>
      teamFilter === "non-daam"
        ? { ...data, items: data.items.filter((s) => s.team !== "DAAM") }
        : data,
  });
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader
        title="SMEs"
        description="DAAM and Non-DAAM subject-matter experts. Profiles drive the matcher."
      />
              ...
              <TableBody>
                {query.data.items.map((s) => (
                  <TableRow key={s.id}>
                    ...
                    <TableCell>
                      <Badge variant={s.team === "DAAM" ? "accent" : "muted"}>{s.team}</Badge>
                    </TableCell>
