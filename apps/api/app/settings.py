...
class Settings(BaseSettings):
    ...
    # ------------------------------------------------------------------
    # MaaS — OpenAI-compatible endpoint (see ADR-0001 + plan 10)
    # ------------------------------------------------------------------
    llm_base_url: str = Field(..., description="MaaS endpoint base URL.")
    llm_api_key: SecretStr = Field(..., description="MaaS API key.")
    llm_chat_model: str = "granite-3-2-8b-instruct"
    ...
    def _reject_placeholder_api_key(cls, value: SecretStr) -> SecretStr:
        if value.get_secret_value() == "changeme":
            raise ValueError(
                ...
                "Provision a real key from your MaaS dashboard."
            )
        return value
