...
llm_calls     — every MaaS call; powers the budget guardrail (plan 10) and the /diagnostics LLM panel (plan 26)
...
class LLMCall(Base):
    """Every MaaS call. NO TimestampedMixin — we use ``created_at`` only
