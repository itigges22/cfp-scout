...
WHAT THIS DOES
    CRUD for the audience profiles conferences and SMEs are described
    with: list with paging, create, rename, delete, each writing an
    audit row, plus the embedding that makes a profile matchable.

HOW IT CONNECTS
    Called by   api/v1/taxonomy.py, app/maintenance.py
    Writes      audiences and their junction tables
    Helpers     services/records.py (paging + audit), services/embeddings.py

WORTH KNOWING
    A twin topic vocabulary lived here too — machine-extracted, 130+
    entries, and nobody ever tagged themselves with it, so the matcher
    dimension it fed dropped for every SME on every score. Removed;
    free-text expertise embedded with the bio carries topical signal.
"""
...
log = structlog.get_logger("scout.taxonomy")
...
def audience_embed_text(a: AudienceProfile) -> str:
