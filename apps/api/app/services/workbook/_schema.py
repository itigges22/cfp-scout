...
# Settings is special: rows are identified by `name` (not _scout_id), and
# rows can't be deleted via workbook (use the /settings/tunables UI to
# reset). No _scout_id / _action — diff and apply both branch on
# sheet_name == "Settings" to handle the name-keyed path.
SETTINGS = SheetSpec(
    name="Settings",
    entity="setting",
    description=(
        "All runtime tunables in one place — LLM API keys, matcher weights, "
        "discovery toggles, log level. Edit the `value` column; everything "
        "else is metadata. Rows with a value that matches the current "
        "setting are no-ops, so unchanged rows cost nothing."
    ),
    columns=(
        ColumnSpec(
            "name",
            ...
            required=True,
            max_len=80,
            note="Setting key. DO NOT EDIT — matches Settings().attribute name.",
        ),
        ColumnSpec(
            "value",
            "long_text",
            required=False,
            note=(
                "The value you want this setting to have. Leave blank to "
                "keep the env default (or the previous override) untouched."
            ),
        ),
        ColumnSpec(
            "kind",
            "text",
            note=(
                "Internal type — bool / int / float / str / secret / list_str. "
                "Display-only. Tells you what shape the value column expects."
            ),
        ),
        ColumnSpec(
            "group",
            "text",
            note="Display-only grouping (llm / matcher / discovery / scraper / logging).",
        ),
        ColumnSpec(
            "description",
            "long_text",
            note="Display-only. What this setting does and any limits.",
        ),
        ColumnSpec(
            "restart_required",
            "bool",
            note="Display-only. If TRUE, the change only takes effect after a container restart.",
        ),
        ColumnSpec(
            "is_secret",
            "bool",
            note=(
                "Display-only. TRUE means this is an API key — handle the "
                "workbook file with care (chmod 600, don't commit to git)."
            ),
        ),
    ),
)
...
# Sheets in the canonical order they appear in the workbook. README is
# the first sheet but generated by template.py rather than from a SheetSpec.
# Settings is second so an operator filling in a fresh template hits the
# API-key cell before they get to the reference-data tabs.
