...
#### `audience_profiles`
Red Hat marketing/sales personas.
...
#### `smes`
Subject-matter experts — DAAM members and external collaborators.

| Column | Type | Purpose |
|--------|------|---------|
| `full_name` | text | 3–100 chars |
| `email` | text nullable | RFC5322 validated; only for past-conference matching |
| `team` | text | `DAAM` or known sibling team name |
...
#### `llm_calls`
Every MaaS call. Powers the budget guardrail + the `/diagnostics` LLM panel.
