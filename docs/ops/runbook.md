...
- `docs/ops/secrets.md` — `.env` management + MaaS key rotation
...
- **`LLM_API_KEY is still set to the placeholder 'changeme'`** —
  provision a MaaS key OR set `LLM_DRY_RUN=true` for offline work.
...
1. Check the error string. Common causes:
   ...
   - **429 / rate limited** — MaaS quota exceeded. The client backs off
...
```bash
# 1. Provision a new key from the MaaS dashboard
...
# 5. After confirming the new key works, revoke the old one in MaaS.
```
...
Only relevant when promoting a new model (e.g. when MaaS publishes a
