...
Scout has one truly sensitive secret — the Red Hat MaaS API key — and a
...
|--------|-------|----------------|
| `LLM_API_KEY` | `.env` on the user's machine | Authorizes MaaS spend on your team's bill. Stolen → financial impact. |
...
The MaaS API key is the one that matters financially. The Postgres passwords
...
$EDITOR .env                     # add your MaaS key, postgres password
make up
```
...
## Provisioning a MaaS key

1. Open the Red Hat MaaS dashboard (URL depends on your environment).
2. Create a new key labeled with your name + `scout` (e.g. `itigges22-scout`).
...
5. Hit `/diagnostics` (plan 26, when it lands) or watch `make logs api` to
   confirm MaaS calls work.

## Rotating the MaaS key
...
```bash
# 1. Provision a new key on the MaaS dashboard (label it with a date)
...
# 5. Revoke the OLD key on the MaaS dashboard
```
...
- **MaaS key**: log into the MaaS dashboard, revoke the key, generate a new one.
- **Postgres password**: rotate per "Rotating the Postgres passwords" above.

### 2. Audit what was reachable with the leaked secret
- For MaaS: check the MaaS billing dashboard for unexpected spend in the last 24 hours.
...
7. **MaaS-side**: per-key billing isolation, revocable independently. A leaked Scout key doesn't compromise other apps that share MaaS access.
