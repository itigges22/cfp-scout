...
(scraping, embedding, matching, decay, CFP digests). The api calls Red Hat
MaaS over the OpenAI-compatible API for both chat (`granite-3-2-8b-instruct`)
...
flowchart LR
    ...
    API -.OpenAI-compatible HTTPS.-> MaaS[Red Hat MaaS]
...
- Talks to MaaS via an OpenAI-compatible client. No direct network calls
  to model inference outside of MaaS.
...
| LLM client | `openai` SDK pointed at MaaS base_url | Provider-agnostic |
...
Scout has one financially-sensitive secret (the MaaS API key) and two
...
- **SME** — subject-matter expert (the DAAM team and external collaborators)
- **CFP** — call for papers; submission window for a conference
- **Pillar** — one of Red Hat AI's four strategic pillars
- **Audience** — Red Hat-defined marketing/sales persona
