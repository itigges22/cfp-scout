...
- **Red Hat alignment.** Docling is IBM Research's project, sister to the Granite models we use via MaaS. Same ecosystem, same maintenance trajectory.
...
- **`unstructured.io`** — Considered. Comparable feature set; popular in the LangChain ecosystem. Lost because: not as cleanly aligned with the Red Hat/IBM ecosystem; broader scope means more dep surface (it carries lots of optional integrations); Docling's HybridChunker is purpose-built for RAG in a way unstructured's chunker isn't.
