---
adr: "0001"
title: Route 1 (Hybrid API via LLM API), local install, 2-container architecture
status: accepted
date: 2026-05-21
supersedes: ""
superseded_by: ""
...
# 0001 — Route 1 (Hybrid API via LLM API), local install, 2-container architecture
...
your LLM endpoint for inference.
...
- **Stack**: FastAPI + Postgres 16 (with pgvector) + Vite-built React SPA +
  your LLM endpoint (OpenAI-compatible API) for chat and embeddings.
...
- **Distribution**: end users `git clone`, copy `.env.example` to `.env`,
  add their LLM key, and run `docker compose up` (or `podman compose up`).
...
- LLM API handles model hosting and observability; we never touch GPUs.
- Provider-agnostic LLM client (`openai` SDK + custom base_url) means we can
  swap the LLM API for another OpenAI-compatible endpoint without code changes.
...
- **Route 2 (locally hosted models)** — Lost because: maintaining
  ...
  ownership for every install. LLM API removes all of that.
