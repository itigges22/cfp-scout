---
adr: "0001"
title: Route 1 (Hybrid API via MaaS), local install, 2-container architecture
status: accepted
date: 2026-05-21
supersedes: ""
superseded_by: ""
...
# 0001 — Route 1 (Hybrid API via MaaS), local install, 2-container architecture

## Context

The Phase 1 plan document (`PLANS/DAAM Scout.pdf`) outlines three possible
routes:

- **Route 1** — Hybrid API cloud via MaaS (FastAPI + Postgres + frontend +
  hosted models)
- **Route 2** — Locally hosted (Flask + llama.cpp + a single Qwen-9B model)
- **Route 3** — Fully cloud hosted on OpenShift (multi-replica + vLLM/llm-d)
...
- **Stack**: FastAPI + Postgres 16 (with pgvector) + Vite-built React SPA +
  Red Hat MaaS (OpenAI-compatible API) for chat and embeddings.
...
- **Distribution**: end users `git clone`, copy `.env.example` to `.env`,
  add their MaaS key, and run `docker compose up` (or `podman compose up`).
...
- MaaS handles model hosting and observability; we never touch GPUs.
- Provider-agnostic LLM client (`openai` SDK + custom base_url) means we can
  swap MaaS for another OpenAI-compatible endpoint without code changes.
...
**Neutral**
- We may grow into Route 3 (OpenShift) in Phase 2 if usage justifies it.
...
- **Route 2 (locally hosted models)** — Lost because: maintaining
  ...
  ownership for every install. MaaS removes all of that.
- **Route 3 (fully cloud hosted on OpenShift)** — Lost for Phase 1 because:
...
- `PLANS/DAAM Scout.pdf` — original phase plan
